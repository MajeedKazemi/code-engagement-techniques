...
export const CodingTasks = [
  ...
  new AuthoringTask(
    "2",
    "Write a function dna_sequences(pattern: str) -> list[str] that generates all possible DNA sequences based on a given pattern using a Queue. The function should accept a single string argument (pattern) and return a list of strings. The input pattern consists of the characters A, T, C, G, and a special placeholder *. The * acts as a wildcard that can be replaced by any of the four DNA bases (A, C, G, T). Your function should return a list of strings, each representing a unique DNA sequence obtained by substituting every * in the input pattern with each possible DNA base. Use a Queue data structure to generate and explore all combinations of sequences.",
    task1Code,
    task1Explanation,
    task1Decomposition,
  ),
