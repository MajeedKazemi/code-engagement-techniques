...
export const tech1WarmupCode = `def is_balanced_parentheses(txt: str) -> bool:
    stack = []
    for char in txt:
        if char == '[':
            stack.append(char)
...
export const tech1WarmupExplanation = `def is_balanced_parentheses(txt: str) -> bool: ### Defines a function named \`is_balanced_parentheses\` that takes a string \`txt\` as an argument and returns a boolean value. The function aims to check if the parentheses in the string are balanced.
