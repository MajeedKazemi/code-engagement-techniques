...
export const WriteOver: React.FC<WriteOverProps> = ({
    text,
    tokens,
    taskID,
}) => {
    ...
    useEffect(() => {
        ...
        setErrorIndexes(
            Array.from<number, number[]>({ length: tokens.length }, () => [])
        );
        setCharToToken(
            tokens.map((line, index) => lineCharToToken(index, line.code))
        );
    ...
    const handleKeyPress = (e: KeyboardEvent<HTMLDivElement>) => {
        ...
        if (
            lines[currentLineIndex].original === userInput &&
            e.key !== "Enter"
        ) {
            return;
        }
        ...
        if (
            lines[currentLineIndex].leadSpaces > 0 &&
            lines[currentLineIndex].currentTabs > 0 &&
            e.key != "Tab"
        ) {
            return;
        }
        ...
        if (e.key == "Tab") {
            e.preventDefault();
            const currentChar =
                lines[currentLineIndex].original[userInput.length];
        ...
        if (
            scores[currentLineIndex] <
            50 +
                Math.round(
                    scores[currentLineIndex] *
                        (1 / lines[currentLineIndex].trimmed.length)
                )
        ) {
            setUserInput("");
            ...
            inputRef.current
                ?.querySelectorAll(".shake-char")
                .forEach((element) => element.classList.remove("shake-char"));
            inputRef.current
                ?.querySelectorAll(".highlight-correct-char")
                .forEach((element) =>
                    element.classList.remove("highlight-correct-char")
                );
        ...
        if (e.key === "Enter" && scores[currentLineIndex] > 50) {
            if (lines[currentLineIndex].original === userInput) {
                ...
                inputRef.current
                    ?.querySelectorAll(".shake-char")
                    .forEach((element) =>
                        element.classList.remove("shake-char")
                    );
                inputRef.current = document.getElementById(
                    `line-${currentLineIndex + 1}`
                ) as HTMLDivElement;
                colorizedText[currentLineIndex] = wrapCharsWithSpan(
                    colorizedText[currentLineIndex],
                    errorIndexes[currentLineIndex],
                    "highlight-correct-char"
                );
            }
            e.preventDefault();
            return;
        ...
        if (lines[currentLineIndex].original[userInput.length] !== value) {
            ...
            const nextCharInLine = inputRef.current?.querySelector(
            ...
            if (newErrorTracker[index] > 2) {
                ...
                const highlightedChar = inputRef.current?.querySelector(
                    `.char-${userInput.length}`
                );
                if (highlightedChar)
                    highlightedChar.classList.add("highlight-correct-char");
                setUserInput(
                    (prevInput) =>
                        prevInput +
                        lines[currentLineIndex].original[userInput.length]
                );
                scores[currentLineIndex] -= Math.round(
                    scores[currentLineIndex] *
                        (1 / lines[currentLineIndex].trimmed.length)
                );
        ...
        if (
            ...
            lines[currentLineIndex].original === nextInput
        ) {
            //log to the db
            apiLogEvents(
                context?.token,
    ...
    useEffect(() => {
        ...
        const explainRef = document.getElementById(
            `id-explanation-${currentLineIndex}`
        );
        const cursorRef = document.getElementById("cursorRef");
        const lineExplainRef = document.getElementById(
            `line-explain-ref-${currentLineIndex}`
        );
        if (explainRef && cursorRef && lineExplainRef) {
            ...
            if (
                lineExplainRef.getBoundingClientRect().x >
                explainRef.getBoundingClientRect().x +
                    explainRef.getBoundingClientRect().width
            ) {
                ...
                explainRef.style.left = `${cursorRef?.getBoundingClientRect()
                    .x!}px`;
            }
        }
    ...
    return (
        <>
            <div className="write-over-reset-container">
                {completed && (
                    <div className="write-over-reset-text">
                        You may now hover over each line to see the explanations
                    </div>
                )}
                <button
            ...
            <div className="write-over-wrapper">
                    ...
                    {isReady &&
                        lines.map((line, index) => (
                            <div
                                id={`line-${index}`}
                                key={index}
                                className="write-over-tracker"
                                onMouseEnter={() => {
                                    if (index < currentLineIndex) {
                                        const updatedHoveringStates = [
                                            ...hoveringStates,
                                        ];
                                        for (
                                            let i = 0;
                                            i < hoveringStates.length;
                                            i++
                                        ) {
                                            if (index == i) {
                                                updatedHoveringStates[index] =
                                                    true;
                                            } else {
                                                updatedHoveringStates[i] =
                                                    false;
                                            }
                                        }
                                        setHoveringStates(
                                            updatedHoveringStates
                                        );
                                    }
                                }}
                                onMouseLeave={() => {
                            ...
                            >
                                {hoveringStates[index] && (
                                    <div
                                        className="hoverable-codeline-explanation"
                                ...
                                {index == currentLineIndex ? (
                                    <div className="current-code-container">
                                        <div
                                            ...
                                            className="current-code-line write-over-code-container"
                                        >
                                            <pre className="user-input">
                                                {userInput}
                                                <span
                                                    className="writeover-cursor"
                                                    id="cursorRef"
                                                />
                                            </pre>
                                            <pre
                            ...
                            {line.original.replace(/\t/g, '    ').split('').map((char, i) => <span className={`char-${i}`}>{char}</span>)} */}
                                                ...
                                                >
                                                    {lines[index].leadSpaces >
                                                        0 && (
                                                        <MdKeyboardTab className="tab-sign" />
                                                    )}
                                                </span>
                                                <span
                                                    className="text-span"
                                ...
                                {index == currentLineIndex && (
                                    <div
                                        className="hoverable-codeline-explanation"
                                        style={{ zIndex: 95 - index }}
                                        id={`line-explain-ref-${index}`}
                                    >
                                        <div className="hints-header">
                                            <div className="hint-icon">
                                                <IconsDoc iconName="explaination" />
                ...
                <div className="writeover-hoverable"></div>
