...
const getColorizedText = async (part:string, language = 'python') => {
 ...
 export const HighlightedPart: React.FC<HighlightedPartProps> = ({ part }) => {
  ...
  const getColorizedTextWithoutTab = async (part:string, language = 'python') => {
    const colorized = await monaco.editor.colorize(
      part,  
      language, 
      {}
 ...
 export const HighlightedPartWithoutTab: React.FC<HighlightedPartProps> = ({ part }) => {
    const [highlightedCode, setHighlightedCode] = React.useState('');
    
    React.useEffect(() => {
      let isCancelled = false;
      getColorizedTextWithoutTab(part).then(result => {
        if (!isCancelled) {
          setHighlightedCode(result)
        }
      });
